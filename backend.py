...
class KnowledgeBase:
    ...
    def _load_all_files(self):
        target_dir = self.kb_path if os.path.exists(self.kb_path) else "."
...
def analyze_contract(text: str, contract_value: float = 10000000, term_years: int = 2, industry: str = "default"):
    ...
    return {
        ...
        "clause_analysis": {
            "total": len(clause_result["present_all"]) + len(clause_result["missing_all"]), "present": len(clause_result["present_all"]), 
...
@app.get("/knowledge-base/data")
async def get_kb_data():
    return {"success": True, "files": kb.loaded_files, "data": kb.data}
